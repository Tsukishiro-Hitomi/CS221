...
def formula1a() -> Formula:
    ...
    # BEGIN_YOUR_CODE (our solution is 1 line(s) of code, but don't worry if you deviate from this)
...
def formula2a() -> Formula:
    ...
    def Parent(x, y):
        return Atom("Parent", x, y)  # whether x's parent is y
...
def formula2c() -> Formula:
    # Predicates to use:
    def Male(x):
        return Atom("Male", x)  # whether x is male

    def Parent(x, y):
        return Atom("Parent", x, y)  # whether x has a parent y

    def Father(x, y):
        return Atom("Father", x, y)  # whether x has a father y
...
# 2. Nicole: "It wasn't me, it was Susan!"
