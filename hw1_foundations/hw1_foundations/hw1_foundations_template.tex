...
\begin{enumerate}[label=\alph*.]
  ...
  \mysolution{The time complexity is ${O}(mpn)$. \\
  Explanation: The demension of result matrix is ${m * p}$, and computing each element of the result matrix requires ${n}$ multiplications and ${n-1}$ additions.
  }
...
\begin{enumerate}[label=\alph*.]
  ...
  \mysolution{ \\ \\
  $C= \begin{pmatrix}26 & 22\\ 79 & 44 \end{pmatrix}$ \\ \\
  $s=171$\\ \\
...
\begin{enumerate}[label=\alph*.]
  ...
  \mysolution{$\theta = \frac{\sum_{i=1}^{n}w_{i}x_{i}}{\sum_{i=1}^{n}w_{i}}$ 
  \\
  Note that $f^{''}(\theta)=2\sum_{i=1}^{n}w_i\ >0$, and $f^{'}(\theta)=2(\theta \sum_{i=1}^{n}w_i-\sum_{i=1}^{n}w_{i}x_{i})$} 
