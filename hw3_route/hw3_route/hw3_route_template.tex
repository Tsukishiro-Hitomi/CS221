...
\mysolution{
  The minimum cost is 
  $n + \frac{m(m+1)}{2}$.
  Note that the cost depends on the x-coordinate of the current state,
  so we should move upward when x is 0, where the minimum cost lies. Since the path is
  $(0, 0)\to(0, n)\to(m, n)$, it's unique.
}
...
\textbf{Question:} Run python map\_util.py $>$ readable\_stanford\_map.txt to write a (long-ish) file of the possible locations on
  ...
  \mysolution{
     NOT FINISHED since I'm not a Stanford student. But the process of visualization is quite funny.
     Below is an example:
      \begin{center}
      \includegraphics[width=0.8\textwidth]{problem-2b.png}
      \end{center}
  }
...
\textbf{Question:} Your system now allows anyone to find the shortest path between any pair of locations on campus. By shortening their travel distance, it promises to optimize travel efficiency.  But, there might be issues when a large fraction of the population start using your system to plan their routes.
  ...
  \mysolution{\\
   1) For system users: When a large number of users follow the same "shortest path" recommended by the system, 
   the origin path may become congested;\\
   2) Their origin preferred path may become worse(more congested or more dirty), which creates a negative
   consumption since non-users bear the costs while not benifiting from the route-planning tool.
   \\My suggestion: The route-planning tool should provide the funtion of visualizing the real-time congestion, which will 
   avoid those users followingthe same "shortest path".
  }
...
\textbf{Question:} If there are $n$ locations and $k$ waypoint tags, what is the maximum possible number of states given a suitable state defintion
  ...
  \mysolution{
    $n \cdot{2^k}$.\ For each location, there are $2^k$ different possible memories considering the waypoints they contain.
  }
...
\textbf{Question:} Choose a starting location, set of waypoint tags, and an end tag (perhaps that captures an interesting route planning
  ...
  \begin{itemize}
    \item amenity=food
    \item name=Li Ka Shing Center
    \item name=Medical School Office Building
  \end{itemize}
  \begin{center}
    \includegraphics[width=0.8\textwidth]{problem-3c.png}
  \end{center}
  }
...
\textbf{Question:} Ride sharing companies use route-finding systems similar to the one you built in this problem to route their drivers. However, these systems have been criticized for using exploitative behavioral nudges. For example, a \href{https://www.nytimes.com/interactive/2017/04/02/technology/uber-drivers-psychological-tricks.html}{New York Times article} reported how one app uses “forward dispatch,” a feature that dispatches a new ride to a driver before the current one ends, to constantly keep drivers busy. This makes it more difficult for drivers to take breaks.
  ...
  \mysolution{I would include waypoints such as restrooms, safe parking lots, low-cost food, gas or charging stations, and quiet rest areas between drop-off and next pick-up so drivers can recover instead of driving continuously. To personalize these waypoints, the system would need driver-specific information such as shift length, fatigue level, medical needs, accessibility needs, and EV battery/fuel status. It may also need preference and safety data, such as preferred neighborhoods, language, and times when a driver wants to avoid high-risk zones. A harmful use would be to insert "profit waypoints" that keep drivers near surge areas while delaying real breaks, nudging them to stay online longer and increasing burnout. Collecting these data also has downsides: privacy invasion, potential discrimination in dispatch decisions, and pressure on drivers to share sensitive health or personal information to remain competitive.}
