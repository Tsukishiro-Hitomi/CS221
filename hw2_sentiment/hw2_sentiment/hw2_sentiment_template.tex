...
\mysolution{$[1, 0, 0, 0, 0, 0, 0, 0, 2, 0, 0, 1, 0, 1].T$}
...
\mysolution{$[0.705, 0.259, 0.035].T$}
...
\mysolution{
\begin{align*}
    \frac{\partial Loss_{CE}}{\partial z_k}
    &= \frac{\partial}{\partial z_k} \left( -\sum_{m=1}^{3}y_m \log p_m \right) \\
    &= -\sum_{m=1}^{3} y_m \frac{\partial \log p_m}{\partial z_k} \\
    &= -\sum_{m=1}^{3} \frac{y_m}{p_m}\frac{\partial \frac{e^{z_m}}{\sum_{m=1}^{3}e^{z_m}}}{\partial z_k} \\
    &= p_k-y_k
\end{align*} 
}
...
\mysolution{
    $[0.2, -0.4].T$\\
    Benifit: easy to compute;\\
    Limitation: easy to be influced by irrelavant words.
...
\hline
$\frac{\partial L}{\partial \hat{y}}$ & $\frac{y_{true}-\hat{y}}{\hat{y}(\hat{y}-1)}$& $2.347$ \\
\hline
$\frac{\partial L}{\partial z}$ &$\frac{\partial L}{\partial y}\*\frac{\partial y}{\partial z}$ & $0.574$ \\
\hline
$\frac{\partial L}{\partial \mathbf{h}}$ &$\frac{\partial L}{\partial y}\frac{\partial y}{\partial z}\frac{\partial z}{\partial \mathbf{h}}$ &$\begin{bmatrix}
    0.459 \\ -0.344
\end{bmatrix}$ \\
\hline
$\frac{\partial L}{\partial \mathbf{W}^{(2)}}$ &$\frac{\partial L}{\partial y}\frac{\partial y}{\partial z}\frac{\partial z}{\partial \mathbf{W^{(2)}}}$ & $\begin{bmatrix}
    0.000 \\ 0.000
\end{bmatrix}$\\
\hline
$\frac{\partial L}{\partial b^{(2)}}$ &$\frac{\partial L}{\partial y}\frac{\partial y}{\partial z}\frac{\partial z}{\partial \mathbf{b^{(2)}}}$ &$0.574$ \\
\hline
$\frac{\partial L}{\partial \mathbf{W}^{(1)}}$ & $\frac{\partial L}{\partial y}\frac{\partial y}{\partial z}\frac{\partial z}{\partial \mathbf{h}} \frac{\partial \mathbf{h}}{\partial \mathbf{W^{(1)}}}$& $\begin{bmatrix}
0.000 & 0.000 \\
0.000 & 0.000
\end{bmatrix}$\\
\hline
$\frac{\partial L}{\partial \mathbf{b}^{(1)}}$ & $\frac{\partial L}{\partial y}\frac{\partial y}{\partial z}\frac{\partial z}{\partial \mathbf{h}} \frac{\partial \mathbf{h}}{\partial \mathbf{b^{(1)}}}$ & $\begin{bmatrix}
    0.000 \\ 0.000
\end{bmatrix}$\\
\hline
\end{tabular}
...
\mysolution{\\1. Positive emotion words (e.g., amazing, love) cluster in the first quadrant (positive coordinates), negative emotion words (e.g., scared, hate, angry) cluster in the third quadrant (negative coordinates), and neutral function words (e.g., this, about, of) lie near the origin (coordinates close to zero), forming distinct semantic clusters in the 2D embedding space.
...
\begin{tabular}{|c|c|c|}
\hline
Learning Rate (lr) & Final Training Loss (Epoch 15) & Validation Accuracy \\
\hline
0.1 & 3.194 & 0.4221 \\
0.2 & 3.266 & 0.4188 \\
0.3 & 3.035 & 0.3869 \\
...
Based on the experiments, here are the observations focusing on three representative learning rates:

\begin{itemize}
    \item \textbf{lr = 0.1}: The training loss decreases very slowly step by step. The final accuracy is relatively low because the small step size makes convergence too slow to reach the optimal point within 15 epochs.
    \item \textbf{lr = 0.5}: The training loss decreases steadily and achieves one of the highest validation accuracies. The step size is appropriate, allowing the model to efficiently descend the loss landscape to a good minimum.
    \item \textbf{lr = 1.0}: The loss drops drastically but the final validation accuracy is degraded. This indicates that the learning rate is too large, causing the parameter updates to "overshoot" the minimum and oscillate, leading to sub-optimal generalization.
\end{itemize}
}
\end{document}