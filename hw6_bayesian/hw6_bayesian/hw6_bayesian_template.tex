...
\begin{shaded}
\textbf{Your Response: }
\text{\text{Rain}$\rightarrow$\text{WetGrass}, \text{Sprinkler}$\rightarrow$\text{WetGrass}}
\end{shaded}
...
\begin{shaded}
\textbf{Your Response:}
\text{$\mathbb{P}(\text{Rain}, \text{Sprinkler}, \text{WetGrass}) = \mathbb{P}(\text{Rain}) \cdot \mathbb{P}(\text{Sprinkler}) \cdot \mathbb{P}(\text{WetGrass}\ | \ \text{Rain}, \text{Sprinkler})$}
\end{shaded}
...
\begin{shaded}
\textbf{Your Response:}
\text{$\mathbb{P}(\text{WetGrass})=\sum_{\text{Rain}, \text{Sprinkler}}\mathbb{P}(\text{Rain}, \text{Sprinkler}, \text{WetGrass})$}
\end{shaded}
...
\textbf{Your Response:}
\begin{align*}
    \mathbb{P}(\text{Rain} = \text{true}, \text{Sprinkler} = \text{true}, \text{WetGrass} = \text{false}) 
    &= \mathbb{P}(\text{Rain} = \text{true}) \cdot \mathbb{P}(\text{Sprinkler} = \text{true}) \\
    &\quad \cdot \mathbb{P}(\text{WetGrass} = \text{false} \mid \text{Rain} = \text{true}, \text{Sprinkler} = \text{true}) \\
    &= 0.2 \cdot 0.5 \cdot (1 - 0.9) \\
    &= 0.01
\end{align*}
\end{shaded}
...
\\ \\
\text{The joint distribution probability is \textbf{$18.2250000000\%$}}
...
Gibbs sampling & 100 & 0.0000 \\
Gibbs sampling & 10{,}000 & 0.0043 \\
Exact & --- & 0.0335 \\
\hline
\end{tabular}
\end{center}
...
\begin{shaded}
\textbf{Your Response:}
\text{\text{Y $\rightarrow$ $A_1$}, \text{Y $\rightarrow$ $A_2$}, \text{Y $\rightarrow$ $A_3$}}
\end{shaded}
...
\textbf{Your Response:}
\begin{center}
    \includegraphics[width=0.7\textwidth]{plots/annotators.png}
\end{center}
\text{It's easy to find that $A_2$ is the least trustworthy.}
\end{shaded}
...
\begin{center}
    \includegraphics[width=0.7\textwidth]{plots/annotators_em.png}
\end{center}

\begin{center}
    \includegraphics[width=0.7\textwidth]{plots/labels_em.png}
\end{center}
...
\begin{shaded}
\textbf{Your Response:}
Yes, EM successfully recovered the latent labels. Although the ground truth labels were hidden, the final distribution of estimated $P(good)$ shows a clear separation between the samples that were actually "good" (clustering around 0.6-0.7) and those that were actually "bad" (clustering around 0.3-0.4). The fact that the annotator CPTs converged to values reflecting different levels of accuracy (similar to the MLE results with labels) further indicates that the model has learned which annotators are more reliable and adjusted the latent label estimates accordingly.
\end{shaded}
